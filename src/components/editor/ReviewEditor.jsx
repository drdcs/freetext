...
export default function ReviewEditor() {
    ...
    useEffect(() => {
        if (editorRef.current && activeTab && editorRef.current.innerHTML !== (activeTab.reviewText || '')) {
            editorRef.current.innerHTML = activeTab.reviewText || '';
        }
    }, [activeTab?.id]);
    ...
    const handleInput = () => {
        if (editorRef.current) {
            ...
            if (content !== activeTab.reviewText) {
                updateTabContent(activeTab.id, { reviewText: content });
            }
        }
    };
        ...
        <div className="rich-text-container h-full flex flex-col">
            ...
            <div className="editor-wrapper flex-1 overflow-auto">
                <div
                    ...
                    onInput={handleInput}
                    onBlur={handleInput}
                    data-placeholder="Document trade-offs, bottlenecks, scaling improvements, review notes, and future enhancements..."
                />
            </div>
        </div>
    );
